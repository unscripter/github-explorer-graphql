...
<div id="content">
  <h1>Timeline for</h1>
  <div v-if="parentData.edges">
  <ul class="timeline" v-for="issue of parentData.edges" :key="issue.cursor">
    <li class="event" data-date="65Million B.C.">
      <h3>commented on {{issue.node.repository.name}} on {{issue.node.issue.createdAt}}</h3>
      <p>{{issue.node.issue.bodyText}}</p>
    </li>  
  </ul>
  </div>
</div>
